...
contract BorrowingVault is BaseVault {
  ...
  ///////////////////////////////*/
    ...
    /// @inheritdoc IVault
    function balanceOfDebtShares(
        address owner
    ) external view override returns (uint256 debtShares) {
    ...
    function _withdraw(
        ...
        uint256 assets,
        uint256 shares
    ) internal override whenNotPaused(VaultActions.Withdraw) {
  ...
  ///////////////////////*/
    ...
    function increaseBorrowAllowance(
        ...
        return
            VaultPermissions.increaseBorrowAllowance(
                operator,
                receiver,
                byAmount
            );
    ...
    function decreaseBorrowAllowance(
        ...
        return
            VaultPermissions.decreaseBorrowAllowance(
                operator,
                receiver,
                byAmount
            );
    ...
    function _computeMaxBorrow(
        ...
        uint256 baseUserMaxBorrow = ((assets * maxLtv * price) /
            (1e18 * 10 ** decimals()));
    ...
    function _computeFreeAssets(
        address owner
    ) internal view override returns (uint256 freeAssets) {
        ...
        if (debtShares == 0) {
            ...
            uint256 lockedAssets = (debt * 1e18 * price) /
                (maxLtv * 10 ** _debtDecimals);
    ...
    function _borrow(
        ...
        uint256 assets,
        uint256 shares
    ) internal whenNotPaused(VaultActions.Borrow) {
    ...
    function _payback(
        ...
        uint256 assets,
        uint256 shares
    ) internal whenNotPaused(VaultActions.Payback) {
  ...
  /////////////////*/
    ...
    ) external hasRole(msg.sender, REBALANCER_ROLE) returns (bool) {
        if (
  ...
  ////////////////////*/
    ...
    function getLiquidationFactor(
        address owner
    ) public view returns (uint256 liquidationFactor) {
        ...
        returns (uint256 gainedShares)
    {
        ...
        uint256 debtSharesToCover = Math.mulDiv(
            _debtShares[owner],
            liquidationFactor,
            1e18
        );
        ...
        uint256 gainedAssets = Math.mulDiv(
            debtToCover,
            10 ** _asset.decimals(),
            discountedPrice
        );
