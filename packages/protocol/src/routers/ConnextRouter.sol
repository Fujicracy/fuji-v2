...
contract ConnextRouter is BaseRouter, IXReceiver {
  ...
  ////////////////////////////////////*/
    ...
    function _accountForSlippage(
        ...
        bytes memory args,
        uint256 slippageThreshold
    ) internal pure returns (bytes memory newArgs, uint256 beforeSlipped) {
        newArgs = args;
    ...
    function _crossTransfer(
        ...
        (
            ...
            address delegate
                ...
                params,
                (uint256, uint256, address, uint256, address, address, address)
            );
    ...
    function _crossTransferWithCalldata(
        bytes memory params,
        address beneficiary
    ) internal override returns (address beneficiary_) {
        (
            uint256 destDomain,
            uint256 slippage,
            address asset,
        ...
        (Action[] memory actions, bytes[] memory args, ) = abi.decode(
            callData,
            (Action[], bytes[], uint256)
        );

        beneficiary_ = _checkBeneficiary(
            beneficiary,
            _getBeneficiaryFromCalldata(actions, args)
        );
    ...
    function _getBeneficiaryFromCalldata(
        Action[] memory actions,
        bytes[] memory args
    ) internal view returns (address beneficiary_) {
        if (actions[0] == Action.Deposit || actions[0] == Action.Payback) {
            // For Deposit or Payback.
            (, , address receiver, ) = abi.decode(
                args[0],
                (IVault, uint256, address, address)
            );
            beneficiary_ = receiver;
        } else if (
            actions[0] == Action.Withdraw || actions[0] == Action.Borrow
        ) {
            ...
            actions[0] == Action.PermitBorrow ||
            actions[0] == Action.PermitWithdraw
        ) {
            (, address owner, , , , , , ) = abi.decode(
                args[0],
                (
            ...
            (, , , , bytes memory requestorCalldata) = abi.decode(
                args[0],
                (IFlasher, address, uint256, address, bytes)
            );
