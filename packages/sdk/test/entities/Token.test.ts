...
describe('Token', () => {
  ...
  describe('#constructor', () => {
    ...
    it('fails with negative decimals', () => {
      expect(() => new Token(3, ADDRESS_ONE, -1, SYMBOL).address).toThrow(
        'DECIMALS'
      );
    });
    it('fails with 256 decimals', () => {
      expect(() => new Token(3, ADDRESS_ONE, 256, SYMBOL).address).toThrow(
        'DECIMALS'
      );
    });
    it('fails with non-integer decimals', () => {
      expect(() => new Token(3, ADDRESS_ONE, 1.5, SYMBOL).address).toThrow(
        'DECIMALS'
      );
    });
  });
  ...
  describe('#equals', () => {
    it('fails if address differs', () => {
      expect(
        new Token(1, ADDRESS_ONE, 18, SYMBOL).equals(
          new Token(1, ADDRESS_TWO, 18, SYMBOL)
        )
      ).toBe(false);
    });
    ...
    it('false if chain id differs', () => {
      expect(
        new Token(3, ADDRESS_ONE, 18, SYMBOL).equals(
          new Token(1, ADDRESS_ONE, 18, SYMBOL)
        )
      ).toBe(false);
    });
    ...
    it('true if only decimals differs', () => {
      expect(
        new Token(1, ADDRESS_ONE, 9, SYMBOL).equals(
          new Token(1, ADDRESS_ONE, 18, SYMBOL)
        )
      ).toBe(true);
    });
    ...
    it('true on reference equality', () => {
      const token = new Token(1, ADDRESS_ONE, 18, SYMBOL);
